/* Molten-metal hero shader — raw WebGL, no dependencies.
   Domain-warped fbm driven through a black→ember→gold heat ramp;
   pointer position injects a local heat bloom. */
...
void main() {
  ...
  // molten flow: two-pass domain warp drifting upward like rising heat
  vec2 q = vec2(fbm(p * 2.1 + vec2(0.0, -t * 1.6)),
                fbm(p * 2.1 + vec2(5.2, 1.3 - t)));
  vec2 r = vec2(fbm(p * 2.1 + 3.4 * q + vec2(1.7, 9.2)),
                fbm(p * 2.1 + 3.4 * q + vec2(8.3, 2.8) + t * 0.4));
  float f = fbm(p * 2.1 + 3.2 * r);

  // seams of heat between plates of cooled metal
  ...
  // deepest heat pools at the bottom of the viewport
...
export function initForgeShader(canvas, reducedMotion) {
  ...
  // render at reduced density — fbm cost dominates, chrome upscales cleanly
  ...
  let mx = 0.5, my = 0.35, heat = 0, targetHeat = 0;
  ...
  window.addEventListener("pointerleave", () => { targetHeat = 0; });
